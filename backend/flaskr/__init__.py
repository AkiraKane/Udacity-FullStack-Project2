...
def create_app(test_config=None):
  ...
  def retrieve_questions():
    ...
    return jsonify({
      ...
      "categories": categories_dict,
      "current_category": [ str(current_question["category"]) for current_question  in current_questions ]
    })
  ...
  def create_question():
    ...
    # If there is no search term, we simple add new question to the db from form input
    ...
    # if we do have search term
      ...
      selections = Question.query.filter(Question.question.ilike(f"%{search_term}%")).all()
  ...
  def get_questions_by_category(id):
    ...
    selections = Question.query.filter(Question.category == str(category.id)).all()
  ...
  def play_quizzes():
    ...
    if quiz_category_id == 0:
      next_question = Question.query.filter(Question.id.notin_(previous_questions)).first()
    else:
      next_question = Question.query.filter(Question.id.notin_(previous_questions)).filter_by(category=quiz_category_id).first()
